 ...
 * 
 * Description: This is a program that represents a fictional CPU
 *              called Coffee Cake.
 *
 */
  ...
  *instrs_changed = 0;
  ...
  for (i = 0; i < instr_num; i++) {
    ...
    if (opcode == BR || opcode == LW || opcode == SW) {
      ...
      /*Make sure new address is divisible by 4, less than 12288 
        and more than 0 */
      if (address % 4 || address >= NUM_BYTES || address < 0) {
        return 0;
      }
